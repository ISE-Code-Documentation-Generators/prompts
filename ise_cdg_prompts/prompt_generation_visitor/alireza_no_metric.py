...
class AlirezaNoMetricPromptGenerator(PromptGenerationVisitor):
    def visit_task(self, task: "Task") -> str:
        return (
            "Give me the summary of the code from the corresponding code. Codes are delimited by triple backquotes and summaries are delimited by triple quotes:\n"
            + self.visit_templates(task.templates)
            + f'\nCode {len(task.templates) + 1}: ```{task.question.code}```\nSummary {len(task.templates) + 1}:'
        )
